/*
 * Test Vehicle — Phase 1 Sketch  (rev 3)
 ...
 *
 * Rev 2 changes:
 *   - JOY_MAX set to 98 (observed physical maximum) for full PWM range
 *   - Explicit stopMotors() when joystick is centered (cleaner stop)
 *   - Right motor direction inverted (motors mounted mirror-image on chassis)
 * Rev 3 changes:
 *   - Pivot turn mixing: inner wheel stops instead of reversing on pure turns
 *     Forward=both drive, Left=Motor1 stop/Motor2 fwd, Right=Motor1 fwd/Motor2 stop
 *
 * Library required:
...
void setup() {
  ...
  Serial.println(F("=== Test Vehicle Phase 1 Boot (rev 3) ==="));
